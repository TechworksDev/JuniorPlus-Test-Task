# JuniorPlus-Test-Task

Тестовое задание для Junior+ разработчика.

### !!! Ограничения по выполнению задания !!!

Тестовое задание должно быть выполнено самостоятельно, без использования AI-ассистентов и инструментов автоматической генерации кода (ChatGPT, GitHub Copilot, Claude, Gemini и т.п.).

Допускается использование официальной документации, справочных материалов и открытых источников (MDN, docs PostgreSQL, Express, Vue и т.д.).

Использование AI-инструментов для генерации кода, архитектурных решений или текстов задания не допускается.

## Задача

Реализовать простую платформу для заметок (**Notes Platform**) с backend- и frontend-частью.

⏱ Срок исполнения: **1 день**

---

## Основные требования

### 1. База данных

- 1.1. Спроектировать модель данных самостоятельно.
- 1.2. Поднять базу **PostgreSQL** в Docker.
- 1.3. Приложить **скриншот модели БД**.

### 2. Backend

- 2.1. Использовать **TypeScript + Express**.
- 2.2. Реализовать API для работы с заметками (состав методов и сущностей определить самостоятельно).
- 2.3. Добавить middleware для логирования и обработки ошибок.
- 2.4. Сделать валидацию входных данных.
- 2.5. Подготовить документацию API в **Swagger**.

### 3. Frontend

- 3.1. Использовать **Vue 3 (TypeScript)**.
- 3.2. Реализовать интерфейс для работы с заметками, подключив его к backend API.

---

## Будет плюсом

- Написание миграций для создания таблиц в БД.
- Поднять сервисы в **docker-compose** (PostgreSQL + backend + frontend).
- Оформить проект с понятной структурой.

---

## Структура проекта

```
JuniorPlus-Test-Task/
├── backend/                 # Backend приложение
│   ├── src/
│   │   ├── config/         # Конфигурация (Swagger)
│   │   ├── controllers/    # Контроллеры
│   │   ├── db/             # Подключение к БД
│   │   ├── middleware/     # Middleware (logger, error handler)
│   │   ├── models/         # Модели данных
│   │   ├── repositories/   # Репозитории для работы с БД
│   │   ├── routes/         # Маршруты API
│   │   └── validators/     # Валидаторы
│   ├── migrations/         # SQL миграции
│   ├── Dockerfile
│   └── package.json
├── frontend/               # Frontend приложение
│   ├── src/
│   │   ├── services/       # API сервисы
│   │   ├── types/          # TypeScript типы
│   │   ├── App.vue         # Главный компонент
│   │   └── main.ts         # Точка входа
│   ├── Dockerfile
│   └── package.json
├── docker-compose.yml      # Docker Compose конфигурация
└── README.md
```

---

## Модель базы данных

### Таблица `notes`

| Поле         | Тип                   | Описание                        |
| ------------ | --------------------- | ------------------------------- |
| `id`         | SERIAL PRIMARY KEY    | Уникальный идентификатор        |
| `title`      | VARCHAR(255) NOT NULL | Заголовок заметки               |
| `content`    | TEXT                  | Содержимое заметки              |
| `created_at` | TIMESTAMP             | Дата создания (автоматически)   |
| `updated_at` | TIMESTAMP             | Дата обновления (автоматически) |
